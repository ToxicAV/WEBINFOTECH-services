<?php
/**
 * Meridian Home Services — block theme functions.
 *
 ...
 * theme.json can't express, and (2) registering the "Meridian: Estimator"
...
define( 'MERIDIAN_THEME_VERSION', '2.0.0' );
...
function meridian_enqueue_assets() {
	wp_enqueue_style( 'meridian-theme-style', get_stylesheet_uri(), array(), MERIDIAN_THEME_VERSION );
	...
	wp_enqueue_style(
		'meridian-theme-support',
		get_template_directory_uri() . '/assets/css/theme.css',
		array(),
		MERIDIAN_THEME_VERSION
	);
}
...
add_action( 'wp_head', 'meridian_head_extras', 1 );
 ...
 */
function meridian_register_blocks() {
	register_block_type( __DIR__ . '/blocks/estimator' );
}
add_action( 'init', 'meridian_register_blocks' );

/**
 * Custom block category so the Estimator block is easy to find in the
 * block inserter.
 */
function meridian_block_categories( $categories ) {
	return array_merge(
		array(
			array(
				'slug'  => 'meridian',
				'title' => __( 'Meridian', 'meridian-home-services' ),
			),
		),
		$categories
	);
